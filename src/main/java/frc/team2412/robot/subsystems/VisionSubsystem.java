...
//  *
//  * <p>2D field poses are different. +X is away from the driver and +Y is toward the opposing
// loading
...
// declaration (private static final AprilTagFieldLayout fieldLayout = ...;). Instead, we have to
// initialize it in a static initializer (static { ... }).
// 	 */
...
// 		 *
// 		 * We have to use temp so that the compiler knows we initialize fieldLayout exactly once. If we
// directly initialize it, the compiler thinks we might initialize it twice if we get an IOException
// after initializing it in the try block (since then we'd run the catch block and initialize there
// too).
// 		 */
...
// 	/**
// 	 * Calculates the robot pose using the best target. Returns null if there is no known robot
// pose.
// 	 *
